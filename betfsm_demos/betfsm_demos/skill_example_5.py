...
from betfsm import (
    ...
    EventSequential, EventConcurrent, ctrl_c_polling_func, get_path_value,
    TimedWait
)
...
class OpeningGripper(Sequence):
    def __init__(self):
        super().__init__("open_gripper",[
            Message(msg="Start to open"),
            TimedWait("waiting_on_gripper",3),
            Message(msg="gripper open")
        ])
...
def main(args=None):
    ...
    open_sm = OpeningGripper()

    sm = EventConcurrent("check_cancel", ctrl_c_polling_func("CTRL_C",3),{NO_EVENT:nominal_sm, "CTRL_C":open_sm})
