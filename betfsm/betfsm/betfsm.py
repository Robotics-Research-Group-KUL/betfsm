...
class Event(GeneratorWithList):
    ...
    Checks for events using an EventQueueSubscriber (per topic)

    Design:

    - Events are specified using a  Dict that maps events to outcomes
        (if priorities are needed to first handle specific events, use more than one node)
    
    - Factor out the Topic and crospi stuff, using a callback function to read possible events
    one at a time
    ...
    event_map: 
      ...
      - All subtrees will be executed as **ConcurrentSequence**:
          ...
          - finishes when all active subtrees have returned SUCCEED; and returns SUCCEED
          - finishes when a subtree or event returns something else than TICKING or SUCCEED, and 
            returns that outcome.
          - if not specified, NO_EVENT = TICKING, i.e. receiving no event, will cause the Event to wait
            and poll the event queue at sampling frequency.          
          - subtrees can become active when triggered by evenstring or NO_EVENT for the nominal state machine.            
          - While  a subtree is active is does not listen any more to the event corresponding
            to that subtree.  Once it returns SUCCEED, it can again react to the event.
